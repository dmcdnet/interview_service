...
public class InterviewQuestion extends InterviewNode {
    ...
    public InterviewNode split(boolean splitRoot) {
        ...
        if (!this.getSplitHistory().isEmpty()) {
            return null;
        }
        return new InterviewQuestion(this.getId(), this.getParent(),
                this.getSplit() != null ? this.getSplit().clone() : null,
                this.getQuestion(), new HashSet<>());
    }
